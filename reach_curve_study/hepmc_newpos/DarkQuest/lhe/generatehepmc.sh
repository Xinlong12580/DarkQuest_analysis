...
<<COMMENT
> "Eta_electrons_3300_3800.txt"
{
for eps in "${Alleps[@]}"; do
	for emass in "${Allemass[@]}"; do
		if [ -e ./e/eta/m_$emass.lhe ]; then
			./bin/displacedHepmc e/eta/m_$emass.lhe Eta_"$emass"_$eps.root Eta electron 0 $eps $emass 3300 3800 1 | tee -a "Eta_electrons_3300_3800.txt"		
		fi
	done
done
}
COMMENT
> "Pion_electrons_3300_3800.txt"
{
for eps in "${Alleps[@]}"; do
	for emass in "${Allemass[@]}"; do
		if [ -e ./e/pion/m_$emass.lhe ]; then
			./bin/displacedHepmc e/pion/m_$emass.lhe Pion_"$emass"_$eps.root Pion electron 0 $eps $emass 3300 3800 1 | tee -a  "Pion_electrons_3300_3800.txt"	
		fi
	done
done
}

> "Eta_muons_3300_3800.txt"
{
for eps in "${Alleps[@]}"; do
	for mumass in "${Allmumass[@]}"; do
		if [ -e ./mu/eta/m_$mumass.lhe ]; then
			./bin/displacedHepmc mu/eta/m_$mumass.lhe Eta_"$mumass"_$eps.root Eta muon 0 $eps $mumass 3300 3800 1 | tee -a "Eta_muons_3300_3800.txt"		
		fi
	done
done
}
