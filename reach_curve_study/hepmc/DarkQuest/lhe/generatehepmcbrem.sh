...
for eps in "${Alleps[@]}"; do
	for emass in "${Allemass[@]}"; do
		...
		if [ -e ./e/brem/SpinQuestAprimeToElectronsLHE_Brem_mAp_"$emass"_GeV.txt ]; then
			./bin/displacedHepmc ./e/brem/SpinQuestAprimeToElectronsLHE_Brem_mAp_"$emass"_GeV.txt Brem_"$emass"_$eps.root Brem electron 0 $eps $emass 500 600 0 | tee -a "Brem_electrons_500_600.txt"		
		fi
	done
done
}
COMMENT
> "Brem_muons_500_600.txt"
{
for eps in "${Alleps[@]}"; do
	for mumass in "${Allmumass[@]}"; do
		...
		if [ -e ./mu/brem/SpinQuestAprimeToMuonsLHE_Brem_mAp_"$mumass"_GeV.txt ]; then
			./bin/displacedHepmc ./mu/brem/SpinQuestAprimeToMuonsLHE_Brem_mAp_"$mumass"_GeV.txt Brem_"$mumass"_$eps.root Brem muon 0 $eps $mumass 500 600 0 | tee -a "Brem_muons_500_600.txt"		
		fi
	done
done
}
