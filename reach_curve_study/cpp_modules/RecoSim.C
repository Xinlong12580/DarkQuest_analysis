...
int RecoSim(std::string top_dir, std::string data_type, const int nevents = 5,
		 std::string in_file="$HepMC_DIR/Brem_0.550000_z500_600_eps_-5.4.txt",
		 std::string out_file="./output.root",
		 std::string config_file="support/RecoSim_config.ini",
		 bool grid_on=false
	   )
{
    ...
    int file_tag1=dist_em(gen);
    int file_tag2=dist_em(gen);
    std::stringstream ss1;
    ss1<< std::setw(4)<<std::setfill('0')<<file_tag1;
    std::string embedding_file1=std::string("/pnfs/e1039/persistent/users/kenichi/data_emb_e906/")+ss1.str()+"/embedding_data.root";
    std::stringstream ss2;
    ss2<< std::setw(4)<<std::setfill('0')<<file_tag2;
    std::string embedding_file2=std::string("/pnfs/e1039/persistent/users/kenichi/data_emb_e906/")+ss2.str()+"/embedding_data.root";
    if(grid_on){
	    embedding_file1="./embedding/file1.root";
	    embedding_file2="./embedding/file2.root";
	   }
    const int verbosity=0;
    int isim=3;
    int igun=10;
    if(data_type=="AprimeSignalMu-Sim"){
	    isim=1;
    }
    if(data_type=="AprimeSignalE-Sim"){
	    isim=2;
    }
    else if (data_type=="DimuonParticle-Sim"){
	    isim=9;
    }
    else if (data_type=="Background-pythiaGenSim"){
	    isim=8;
    }
    // input simulation
    ...
    rc->set_IntFlag("MaxHitsDC0" , int(350)); // number of hits per chamber. Suggestion make it half
  ...
  rc->set_IntFlag("MaxHitsDC3m", int(140));
    if (doEMCal)
    {
        rc->set_CharFlag(
            "AlignmentMille",
            "$DIR_TOP/support/data/alignment/align_mille.txt"); // alignment file needed for EMCAL
    }
    ...
    if (do_aprime_muon || do_aprime_electron){
     	DoEmbedding *do_emb=new DoEmbedding();
     	do_emb->Verbosity();
     	do_emb->AddEmbDataFile(embedding_file1.c_str());
     	do_emb->AddEmbDataFile(embedding_file2.c_str());
     	int n_evt_emb=do_emb->GetNumEmbEvents();
     	//se->registerSubsystem(do_emb);
	//std::cout<<std::endl<<embedding_file1<<std::endl<<std::endl;
	//return 0;
    ...
    dptrigger->set_dproad_set_file_name("$DIR_TOP/support/DPTrigger_road29.txt");
    //dptrigger->set_dproad_set_file_name("$DIR_TOP/support/DPTrigger_road16.txt");
    ...
    if (do_aprime_muon or do_aprime_electron)
    {
        ...
        in->fileopen(gSystem->ExpandPathName(in_file.c_str()));
