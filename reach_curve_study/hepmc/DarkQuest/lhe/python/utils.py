...
def plotBR():
    ...
    for idx, lep in enumerate(['electrons', 'muons']):
        ...
        ax.plot(masses, bfs, label=lep, color=colors[idx], marker=markers[idx],markersize=2.3,linewidth=0.5)
...
def loadRates(epsilon, POT = 1.44e18, mech="Brem"):
    ...
    assert mech in ["Brem", "Eta", "Pion"], "Error: unknown production mechanism. Choose from Brem or Eta"
...
def plotRates(POT = 1.44e18):
    ...
    for mech in ['Brem', 'Eta','Pion']:
        for epsilon in [1.0e-6]:#, 1.0e-7, 1.0e-8]:
            ...
            ax.plot(masses, rates, label=f"{mech}, $\epsilon={epsilon}$", color=colors[idx], marker=markers[idx],markersize=2.3,linewidth=0.5)
    ...
    ax.set_yscale('log')
    ax.set_xlabel('$m_{(A^{\prime})}$ [GeV]')
    ax.set_ylabel('Rate ($A^{\prime}$ / 1e17 POT)')
    ax.legend()
    plt.show()
    fig.savefig(f"Rates.pdf")
